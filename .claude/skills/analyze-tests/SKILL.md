...
2. Replace the `tests` list with the candidate inline forms (one string
   per test, written as it would appear on a single line).
...
Each line shows the total character count, a `SINGLE`/`MULTI` mark, and
a preview of the test string:
