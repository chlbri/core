...
repo. It is meant to be used on every successful merge to master but
you'll need to configure that workflow yourself. You can look to the
[`.github/workflows/push.yml`](./.github/workflows/push.yml) file in
...
[#193](https://github.com/phips28/gh-action-bump-version/issues/193)),
if someone uses windows, and wants to run this action, happy to review
your PR ;)
...
- Based on the commit messages, increment the version from the latest
  ...
  - If the string "BREAKING CHANGE", "major" or the Attention pattern
    ...
    commit messages or descriptions the major version will be
    incremented.
  - If a commit message begins with the string "feat" or includes
    "minor" then the minor version will be increased. This works for
    most common commit metadata for feature additions: `"feat: new API"`
    and `"feature: new API"`.
...
Customize the messages that trigger the version bump. It must be a
string, case sensitive, comma separated (optional). Example:
...
Set a default version bump to use (optional - defaults to patch).
Example:
...
Set a preid value will building prerelease version (optional - defaults
to 'rc'). Example:
...
No commit is made after the version is bumped (optional). Must be used
in combination with `skip-tag`, since if there's no commit, there's
nothing to tag. Example:
...
Set a custom target branch to use when bumping the version. Useful in
cases such as updating the version on master after a tag has been set
(optional). Example:
...
Set version bump ignore policy. Useful for pull requests between
branches with version bumps. Options are as follows:
...
Set check-last-commit-only to only read last commit's message
(optional). Example:
