...
export const castings = expandFn(<T>(...values: T[]) => values, {
  ...
  tuple: expandFn(
    ...
    {
      multiply: <const T, N extends number>(data: T, times: N) => {
      ...
      is: <const T>(fn: Checker2<T>) => {
        const _out = (value: unknown): value is Array<T> => {
          return castings.is(value) && value.every(fn);
        };
  ...
  freeze: <const T extends any[]>(...args: T) =>
    Object.freeze(castings.tuple(...args)),
